...
for (gmt_name in names(gmt_files)) {
  message("Running ssGSEA for: ", gmt_name)
  gene_sets <- getGmt(gmt_files[[gmt_name]])
  ...
  # Filenames for scores
  rds_name <- if(gmt_name == "hallmark") "ssgsea_hallmark_scores.rds" else paste0("ssgsea_", gmt_name, "_scores.rds")
  csv_name <- if(gmt_name == "hallmark") "ssgsea_scores_matrix.csv" else paste0("ssgsea_", gmt_name, "_scores_matrix.csv")
  
  saveRDS(ssgsea_res_all, paste0("results/ssgsea/", rds_name))
  write.csv(as.data.frame(ssgsea_res_all), paste0("results/ssgsea/", csv_name))

  # Limma analysis prefix
  file_prefix <- if(gmt_name == "hallmark") "" else paste0(gmt_name, "_")
  ...
  if (nrow(metadata_clusters) > 0) {
    ...
    if (length(levels(group)) >= 2) {
      ...
      for (cl in levels(group)) {
        ...
        write.csv(res, paste0("results/ssgsea/limma_pathway_", file_prefix, clean_name, "_vs_others.csv"), row.names = FALSE)
      }
    }
  }
  ...
  if (nrow(metadata_general) > 0) {
    ...
    if (length(levels(condition)) >= 2) {
      ...
      if ("Sjogrens" %in% colnames(design_sjs) && "Control" %in% colnames(design_sjs)) {
          ...
          write.csv(res_sjs, paste0("results/ssgsea/limma_pathway_", file_prefix, "sjs_vs_ctrl.csv"), row.names = FALSE)
      }
    }
  }
}
